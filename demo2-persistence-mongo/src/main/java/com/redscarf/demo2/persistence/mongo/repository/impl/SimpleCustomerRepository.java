package com.redscarf.demo2.persistence.mongo.repository.impl;
 ...
 * <p>Version: 1.0
 */
public class SimpleCustomerRepository<T, ID extends Serializable> extends SimpleMongoRepository<T, ID> implements CustomerRepository<T, ID> {
    ...
    @Override
    public <S extends T> List<S> saveAll(Iterable<S> list){
        for (S entity : list) {
            if( entity instanceof MongoBaseModel) {
                if( ((MongoBaseModel) entity).getUpdatedAt() == null ){
                    ((MongoBaseModel) entity).setUpdatedAt(new Date());
